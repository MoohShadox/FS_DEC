...
class Wrapper_Simulated_Annealing(Wrapper_Model):
    ...
    def nextSubset(self,subset, *args, **kwargs):
        ...
        best_neighbour = sneighbourhood[0][0]
        delta = -self.aggregator(sneighbourhood[0][1]) + self.aggregator(self.evaluateSubset(subset))
        p = min(1,np.exp(-delta / self.T))
        r = np.random.random()
        self.T = self.T * 0.9
        if(r < p):
...
if __name__ == "__main__":
    ...
    wls = Wrapper_Simulated_Annealing(X,y, evaluation=evaluator.evaluate, aggregator=lambda x:x[0])
